...
2. Keep implementation, `contracts/openapi.json`, `apps/api/schema.sql`, and `architecture.md` in sync.
...
2. Validate compatibility with `contracts/openapi.json`, `apps/api/schema.sql`, and `architecture.md`.
...
- No hidden behavior outside declared contracts (OpenAPI, DB schema).
- No schema or contract drift: update `contracts/openapi.json` via `nx run api:openapi`, and keep `apps/api/schema.sql` and migrations in sync.
